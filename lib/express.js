...
let coininfo = require('./coininfo/lib/coininfo');
...
module.exports = function initialize (adapter, opts) {
  ...
  if (opts.network) {
    let net = coininfo[opts.network]
    if (opts.testnet) {
      network = net.test.toBitcoinJS()
    }
    else if (opts.regtest) {
      network = net.regtest.toBitcoinJS()
    }
    else {
      network = net.main.toBitcoinJS()
    }
  }

  // convenience function to search ranges on leveldb
  let searchRange = (fn, scId, cb) => adapter[fn]({scId, heightRange: [0, 2500000]}, 3000000, cb)
